...
export class DexcomClient {
        ...
        this._log = (message, data = null) => {
            if (this._settings && this._settings.get_boolean('enable-debug-logs')) {
                if (data) {
                    ...
                    console.log(`[DexcomClient] ${message}`, maskedData);
                } else {
    ...
    async _makeRequest(url, method = 'GET', data = null, params = null) {
        try {
            ...
            if (data && method !== 'GET') {
                ...
                this._log(`Request body: ${JSON.stringify(this._maskSensitiveData(data))}`);
            } else {
    ...
    _logDebugInfo(stage, data) {
        ...
        this._log(`[DEBUG ${timestamp}] ${stage}:`, JSON.stringify(this._maskSensitiveData(data), null, 2));
    }
...
async authenticate() {
    try {
        ...
        this._log('Account ID received: ***MASKED***');
